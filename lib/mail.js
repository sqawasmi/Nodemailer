...
function EmailMessage(params){
    ...
    this.inreply_to = params.inreply_to;
...
EmailMessage.prototype.generateHeaders = function(){
    ...
    for(var i=0, len=keys.length; i<len; i++){
        var headerKey = upperFirst(keys[i].trim());
        var headerValue = this.headers[keys[i]];
        headers.push([
            headerKey,
            (hasUTFChars(headerValue) &&
                    mimelib.encodeMimeWord(headerValue, "Q") || headerValue) || ''
        ].join(": "));
    }
    ...
    //In-Reply-To
    var inreply_to = this.generateAddresses(this.inreply_to, 1);
    if(inreply_to.length){
        headers.push([
            upperFirst("In-Reply-To"),
            inreply_to
        ].join(": "));
    }

    //References
    var references = this.generateAddresses(this.references, 1);
    if(references.length){
      headers.push([
        upperFirst('References'),
        references
      ].join(": "));
    }
